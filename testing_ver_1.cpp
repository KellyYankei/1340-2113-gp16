    ...
    while (true){
        ...
        if (commands.size() > 2){
            ...
            if (commands[0] == "q"){
                ...
                if (s.find(commands[0][0]) != -1 && n.find(commands[0][1]) != -1){
                    int key = searchpyramid(pyramid, commands[0][0], int(commands[0][1] - '0'));
                    if (key != -1){
                        pyramid[key].status = 0;
                        set_status(pyramid);
                        cout << CLSCR;
                        printmap(pyramid);
                        print_deck(pos, deck, oppo, show);
                        ...
                        printmap(pyramid);
                        print_deck(pos, deck, oppo, show);
                        cout << RED << commands[0] << " is not available" << RESET << endl;
                    }
                }
            ...
            if (commands[0].size() == 2 && commands[1].size() == 2){
                if (s.find(commands[0][0]) != -1 && n.find(commands[0][1]) != -1 && s.find(commands[1][0]) != -1 && n.find(commands[1][1]) != -1){
                    int key_1 = searchpyramid(pyramid, commands[0][0], int(commands[0][1] - '0'));
                    int key_2 = searchpyramid(pyramid, commands[1][0], int(commands[1][1] - '0'));
                    if (key_1 != -1 && key_2 != -1 && pyramid[key_1].num + pyramid[key_2].num == 9){
                        pyramid[key_1].status = 0; pyramid[key_2].status = 0;
                        set_status(pyramid);
                        cout << CLSCR;
                        printmap(pyramid);
                        print_deck(pos, deck, oppo, show);
                        cout << RED << commands[0] << " & " << commands[1] << " have be eliminated" << RESET << endl;
                    }
                    else{
                        cout << CLSCR;
                        printmap(pyramid);
                        print_deck(pos, deck, oppo, show);
                        cout << RED << commands[0] << " & " << commands[1] << " are not available" << RESET << endl;
                    }
                }
